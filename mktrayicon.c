...
char* strncpy_esc(char *dest, const char *src, size_t n)
{
	...
	while (i < n && src[i] != '\0'){
		if (src[i] == '\''){
			...
			if( (!inQuote && src[i+1] != '\0') || (inQuote && src[i+1] == '\'') ){
				dest[index] = src[i+1];
				index++;
				i+=2; 
			}
			else if (inQuote){ // non escaping '\' in quotes
				dest[index] = src[i];
				index++;
				i++;
			}
	...
	outer: while (1) {
		...
		while (1) {
			...
			case 'm': /* menu */
				...
				int straight = 0;
				int bars = 0;
				int inQuote = 0; // if single quotes are odd -> we are inQuote -> we dont count 
				for (int i = 0; i < len; i++){
					if (param[i] == ',' && param[i-1] != '\\' && !inQuote){
						straight++;
						if(straight == 2)
							break;
					}
					else if (param[i] == '|' && param[i-1] != '\\' && !inQuote){
						straight = 0;
						bars++;
					}
